...
void findBestMove(Board3D &board, int maxDepth, int& bestX, int& bestY, int& bestZ) {
    ...
    std::thread loadingThread([&calculationDone]() {
        auto start = std::chrono::steady_clock::now();
        while (!calculationDone) {
            auto now = std::chrono::steady_clock::now();
            auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - start).count();
            if (elapsed > 1) { // Only show message after 1 second
    ...
    for (int depth = 1; depth <= maxDepth; ++depth) {
        ...
        for (const auto& move : moves) {
            board.at(move.x, move.y, move.z) = COMPUTER;
            int moveVal = minimax(board, 0, false, numeric_limits<int>::min(), numeric_limits<int>::max(), depth, abortSearch);
            board.at(move.x, move.y, move.z) = '-';
        ...
        // If you find the best move, exit early
